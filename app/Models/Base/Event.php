...
abstract class Event implements ActiveRecordInterface
{
    ...
    /**
     * The value for the timelineid field.
     *
     * @var        int
     */
    ...
    /**
     * Get the [timelineid] column value.
     *
     * @return int
     */
    ...
    /**
     * Set the value of [dsc] column.
     *
     * @param string $v new value
     * @return $this|\Event The current object (for fluent API support)
     */
    public function setDsc($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }
        ...
        if ($this->dsc !== $v) {
            $this->dsc = $v;
            $this->modifiedColumns[EventTableMap::COL_DSC] = true;
        }
    ...
    public function setCompany($v)
    {
        ...
        if ($this->company !== $v) {
            $this->company = $v;
            $this->modifiedColumns[EventTableMap::COL_COMPANY] = true;
        }
    ...
    /**
     * Set the value of [timelineid] column.
     *
     * @param int $v new value
     * @return $this|\Event The current object (for fluent API support)
     */
    public function setTimelineid($v)
    {
        if ($v !== null) {
            $v = (int) $v;
        }

        if ($this->timelineid !== $v) {
            $this->timelineid = $v;
            $this->modifiedColumns[EventTableMap::COL_TIMELINEID] = true;
        }

        if ($this->aTimeline !== null && $this->aTimeline->getId() !== $v) {
            $this->aTimeline = null;
        }
    ...
    public function setTypeid($v)
    {
        ...
        if ($this->typeid !== $v) {
            $this->typeid = $v;
            $this->modifiedColumns[EventTableMap::COL_TYPEID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : EventTableMap::translateFieldName('Dsc', TableMap::TYPE_PHPNAME, $indexType)];
            $this->dsc = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : EventTableMap::translateFieldName('Company', TableMap::TYPE_PHPNAME, $indexType)];
            $this->company = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : EventTableMap::translateFieldName('Timelineid', TableMap::TYPE_PHPNAME, $indexType)];
            $this->timelineid = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : EventTableMap::translateFieldName('Typeid', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    {
        if ($this->aTimeline !== null && $this->timelineid !== $this->aTimeline->getId()) {
            $this->aTimeline = null;
        }
        if ($this->aEventType !== null && $this->typeid !== $this->aEventType->getId()) {
            $this->aEventType = null;
        }
    } // ensureConsistency
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->aTimeline !== null) {
                if ($this->aTimeline->isModified() || $this->aTimeline->isNew()) {
                    $affectedRows += $this->aTimeline->save($con);
                }
                $this->setTimeline($this->aTimeline);
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(EventTableMap::COL_DSC)) {
            $modifiedColumns[':p' . $index++]  = 'Dsc';
        }
        if ($this->isColumnModified(EventTableMap::COL_COMPANY)) {
            $modifiedColumns[':p' . $index++]  = 'Company';
        }
        if ($this->isColumnModified(EventTableMap::COL_TIMELINEID)) {
            $modifiedColumns[':p' . $index++]  = 'TimeLineID';
        }
        if ($this->isColumnModified(EventTableMap::COL_TYPEID)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'Title':
                        ...
                        break;
                    case 'Dsc':
                        $stmt->bindValue($identifier, $this->dsc, PDO::PARAM_STR);
                        break;
                    case 'Company':
                        $stmt->bindValue($identifier, $this->company, PDO::PARAM_STR);
                        break;
                    case 'TimeLineID':
                        $stmt->bindValue($identifier, $this->timelineid, PDO::PARAM_INT);
                        break;
                    case 'TypeID':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->aTimeline) {
                ...
                $result[$key] = $this->aTimeline->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
        }
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) {
            $this->setDsc($arr[$keys[2]]);
        }
        if (array_key_exists($keys[3], $arr)) {
            $this->setCompany($arr[$keys[3]]);
        }
        if (array_key_exists($keys[4], $arr)) {
            $this->setTimelineid($arr[$keys[4]]);
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(EventTableMap::COL_DSC)) {
            $criteria->add(EventTableMap::COL_DSC, $this->dsc);
        }
        if ($this->isColumnModified(EventTableMap::COL_COMPANY)) {
            $criteria->add(EventTableMap::COL_COMPANY, $this->company);
        }
        if ($this->isColumnModified(EventTableMap::COL_TIMELINEID)) {
            $criteria->add(EventTableMap::COL_TIMELINEID, $this->timelineid);
        }
        if ($this->isColumnModified(EventTableMap::COL_TYPEID)) {
            $criteria->add(EventTableMap::COL_TYPEID, $this->typeid);
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setDsc($this->getDsc());
        $copyObj->setCompany($this->getCompany());
        $copyObj->setTimelineid($this->getTimelineid());
    ...
    {
        if ($this->aEventType === null && ($this->typeid !== null)) {
            $this->aEventType = ChildEventTypeQuery::create()->findPk($this->typeid, $con);
    ...
    /**
     * Declares an association between this object and a ChildTimeline object.
     *
     * @param  ChildTimeline $v
     * @return $this|\Event The current object (for fluent API support)
     * @throws PropelException
     */
    public function setTimeline(ChildTimeline $v = null)
    {
        if ($v === null) {
            $this->setTimelineid(NULL);
        } else {
            $this->setTimelineid($v->getId());
        }

        $this->aTimeline = $v;

        // Add binding for other direction of this n:n relationship.
        // If this object has already been added to the ChildTimeline object, it will not be re-added.
        if ($v !== null) {
            $v->addEvent($this);
        }
    ...
    /**
     * Get the associated ChildTimeline object
     *
     * @param  ConnectionInterface $con Optional Connection object.
     * @return ChildTimeline The associated ChildTimeline object.
     * @throws PropelException
     */
    public function getTimeline(ConnectionInterface $con = null)
    {
        if ($this->aTimeline === null && ($this->timelineid !== null)) {
            $this->aTimeline = ChildTimelineQuery::create()->findPk($this->timelineid, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aTimeline->addEvents($this);
             */
        }
    ...
    public function clear()
    {
        ...
        if (null !== $this->aTimeline) {
            $this->aTimeline->removeEvent($this);
        }
        $this->id = null;
        $this->title = null;
        $this->dsc = null;
