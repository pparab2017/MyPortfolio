...
abstract class Language implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\LanguageTableMap';
    ...
    /**
     * The value for the logo_img_url field.
     *
     * @var        string
     */
    protected $logo_img_url;
    ...
    /**
     * Initializes internal state of Base\Language object.
     */
    ...
    /**
     * Get the [logo_img_url] column value.
     *
     * @return string
     */
     ...
     * @return $this|\Language The current object (for fluent API support)
     */
    ...
    /**
     * Set the value of [logo_img_url] column.
     *
     * @param string $v new value
     * @return $this|\Language The current object (for fluent API support)
     */
    public function setLogoImgUrl($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }

        if ($this->logo_img_url !== $v) {
            $this->logo_img_url = $v;
            $this->modifiedColumns[LanguageTableMap::COL_LOGO_IMG_URL] = true;
        }
    ...
    public function setDes($v)
    {
        ...
        if ($this->des !== $v) {
            $this->des = $v;
            $this->modifiedColumns[LanguageTableMap::COL_DES] = true;
        }
    ...
    public function setColor($v)
    {
        ...
        if ($this->color !== $v) {
            $this->color = $v;
            $this->modifiedColumns[LanguageTableMap::COL_COLOR] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : LanguageTableMap::translateFieldName('LogoImgUrl', TableMap::TYPE_PHPNAME, $indexType)];
            $this->logo_img_url = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : LanguageTableMap::translateFieldName('Des', TableMap::TYPE_PHPNAME, $indexType)];
            $this->des = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : LanguageTableMap::translateFieldName('Color', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Language'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(LanguageTableMap::COL_LOGO_IMG_URL)) {
            $modifiedColumns[':p' . $index++]  = 'logo_img_url';
        }
        if ($this->isColumnModified(LanguageTableMap::COL_DES)) {
            $modifiedColumns[':p' . $index++]  = 'des';
        }
        if ($this->isColumnModified(LanguageTableMap::COL_COLOR)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'Name':
                        ...
                        break;
                    case 'logo_img_url':
                        $stmt->bindValue($identifier, $this->logo_img_url, PDO::PARAM_STR);
                        break;
                    case 'des':
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(LanguageTableMap::COL_LOGO_IMG_URL)) {
            $criteria->add(LanguageTableMap::COL_LOGO_IMG_URL, $this->logo_img_url);
        }
        if ($this->isColumnModified(LanguageTableMap::COL_DES)) {
            $criteria->add(LanguageTableMap::COL_DES, $this->des);
        }
        if ($this->isColumnModified(LanguageTableMap::COL_COLOR)) {
            $criteria->add(LanguageTableMap::COL_COLOR, $this->color);
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setLogoImgUrl($this->getLogoImgUrl());
    ...
    public function initProjects($overrideExisting = true)
    {
        ...
        $collectionClassName = ProjectTableMap::getTableMap()->getCollectionClassName();

        $this->collProjects = new $collectionClassName;
    ...
    public function setProjects(Collection $projects, ConnectionInterface $con = null)
    {
        ...
        foreach ($projectsToDelete as $projectRemoved) {
            $projectRemoved->setLanguage(null);
        }
